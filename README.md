...
Para auditar formatos alternativos de un mismo extracto: `node scripts/check-import-pairs.mjs <formato-a> <formato-b> [...pares]`. Prueba cada pareja en ambos sentidos sobre `dist`, en cuentas y contextos desechables sin conexiones externas ni trazas. Conserva las selecciones por defecto y comprueba los registros tras guardar y recargar; si no hay filas seleccionadas, solo comprueba el estado guardado. Usa cero como saldo inicial ficticio exclusivamente en la prueba cuando hace falta. Informa de recuentos, cambios en movimientos existentes y campos complementarios. Las correspondencias adicionales son candidatos de diagnóstico, no autorizaciones para fusionar. No implementa enriquecimiento ni modifica el perfil del navegador del usuario.
