...
Se guardan dos fechas: **principal**, la menor entre las disponibles en cada operación, y **secundaria**, la mayor. Se examinan operación/inicio, valor, contabilización y finalización según las columnas del archivo, sin guardarlas como cuatro campos separados. Si solo existe una fecha, la secundaria queda vacía; si dos fechas coinciden se conserva esa coincidencia. Estos nombres expresan una preferencia de presentación, no certifican cuándo comenzó la compra o llegó el dinero. Los saldos siguen siendo los del extracto, aunque la fecha menor cambie su orden de presentación.
...
**Importe original y moneda original** son opcionales y se guardan juntos, con la precisión de esa moneda, sin conversión y sin alterar el importe de la cuenta. Se leen de columnas explícitas y de los bloques reconocidos de N26 y Revolut en PDF. Los símbolos ambiguos o varios importes originales incompatibles se conservan en notas con un aviso, sin asignar una moneda arbitraria. Los campos pueden revisarse, editarse y exportarse a CSV; también se incluyen en snapshots y copias de formato 3.
...
**Contraparte** se rellena con el nombre explícito del archivo o, si falta, con reglas locales para expresiones claras como «Pago de…», «Transferencia a…» y «Compra en…». Los formatos conocidos también identifican el nombre del establecimiento; los casos ambiguos quedan vacíos. La referencia del pago se añade al concepto separada por un punto y un espacio, sin repetirla: `Transferencia a Ana Prueba. Factura 01`. No se guarda un campo de referencia adicional.

Los IBAN no se guardan en los textos de los movimientos: se retiran del concepto, la contraparte y las notas durante la importación y la edición. Las columnas de IBAN de contraparte se ignoran. Las copias que contengan IBAN en esos textos se rechazan sin modificar los datos actuales. No se limpian automáticamente los datos de versiones anteriores.
...
Las operaciones del mismo día conservan su procedencia y un orden persistente, sin inventar horas. Las horas principales del mismo origen y cuenta y la continuidad de los saldos permiten determinar la secuencia; no se comparan como instantes absolutos entre zonas horarias. Ordenar las fechas de forma descendente no demuestra el orden dentro de cada día. Si faltan pruebas o las horas contradicen los saldos, se conserva la posición del archivo como orden provisional y se muestra un aviso. «Más recientes primero» invierte también la secuencia dentro del día. Los filtros no recalculan el saldo ni el orden.

Al importar otro archivo en la misma cuenta y moneda, las coincidencias únicas por fecha, importe, concepto y saldo presente e igual pueden enlazarlo con las operaciones guardadas. No se guarda un identificador bancario externo por movimiento. Las relaciones fiables se conservan separadamente de los desempates provisionales; las contradicciones se avisan y no impiden guardar movimientos válidos. El orden se incluye en las copias. Editar fechas o importes invalida la evidencia afectada; no hay ordenación manual.
...
La búsqueda por palabras incluye concepto, contraparte, notas, rutas completas de categorías y etiquetas, con un vocabulario local de sinónimos para nómina, factura, supermercado y alquiler. No garantiza reconocer todos los comercios o conceptos. La ayuda usa textos revisados sobre funciones reales. Sin modelo vigente preparado, el chat muestra una guía breve y el enlace a IA local. No responde mediante reglas ni descarga modelos automáticamente.
